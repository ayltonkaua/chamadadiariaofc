-- ============================================================================
-- Migration: 013_fix_chamada_idempotency_update.sql
-- Purpose: Implement Last-Writer-Wins (LWW) for idempotency keys over same class/date
...
BEGIN
    ...
    IF v_escola_id IS NULL THEN
        RETURN jsonb_build_object('success', FALSE, 'error', 'escola_id_missing', 'message', 'JWT não contém escola_id');
    END IF;
    ...
    -- ========================================================================
    -- 5. VERIFICAR IDEMPOTÊNCIA COM LAST-WRITER-WINS
    -- ========================================================================
    ...
    IF v_existing_timestamp IS NOT NULL THEN
        -- Tratamento "Last-Writer-Wins". Se for mais velho ou igual, ignoramos!
        ...
        -- Caso \`p_client_timestamp\` > \`v_existing_timestamp\`, DEIXAMOS AVANÇAR
    ...
    -- ========================================================================
    -- 7. REGISTRAR OU ATUALIZAR LOG DE SINCRONIZAÇÃO (UPSERT)
    -- ========================================================================
