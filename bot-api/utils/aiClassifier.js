...
Intenções possíveis:
- "justificar_falta": quer justificar falta, atestado, filho faltou, estava doente
...
- "pe_de_meia": pé-de-meia, pé de meia, programa pé de meia, auxílio estudantil
...
const REGEX_PATTERNS = [
    ...
    { intent: 'pe_de_meia', patterns: [/p[eé]\s*(de|-)?\s*meia/i, /aux[ií]lio/i] },
 ...
 * 
 * Estratégia:
 * 1. Tenta Gemini Flash (mais rápido, gratuito)
 * 2. Se falhar, tenta Groq
 * 3. Se ambos falharem, tenta regex patterns
 * 4. Se nada funcionar, retorna null (caller decide — geralmente mostra menu URA)
 * 
 * @param {string} userMessage - A mensagem do usuário
 * @returns {Promise<{intent: string, confianca: number, source: string} | null>}
 */
...
function getGreetingMessage() {
    const hour = new Date().getHours() - 3; // UTC-3 (Brasília)
